...
function initSwiper() {
  // App-like: 一屏一张卡，纵向上下滑动（类似 TikTok）
...
// ===================== App Share Placeholder =====================
// App 壳里可通过 WKWebView message handler 接管分享。
// Web 里则尝试调用 navigator.share（若可用），否则提示。
(function () {
  const btn = document.getElementById('app-share');
  if (!btn) return;

  btn.addEventListener('click', async () => {
    // 优先交给原生（WKWebView）
    try {
      const mh = window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.share;
      if (mh && mh.postMessage) {
        mh.postMessage({ type: 'share', url: location.href });
        return;
      }
    } catch (_) {}
    ...
    alert('分享功能将由 App 原生实现（Tab B / 右上角）。');
  }, false);
})();